...
export const InventarioService = {
    async getInventarioByProductoId(producto_id: number,empresa_id:number,tienda_id:number):Promise<any> { 
        try {
            const inventario = await postgres_db(TABLAS.INVENTARIO)
                ...
                .andWhere('inventario.tienda_id',tienda_id)
    ...
    async putInventarioByStockAbdPrice(data:IPutInventario[],usuario_id:number):Promise<any> {
        try {
            //Capturara info          
            let result:any[] = [];
            data.map(async (e)=>{
